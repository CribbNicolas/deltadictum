...
export function previousDataDirs({ repoRoot, names, env = process.env }) {
...
export async function adoptHistory(db, { projectId, dataDir, candidates }) {
  ...
  for (const dir of candidates) {
    ...
    let rows = 0;
    try {
      db.prepare('ATTACH DATABASE ? AS previous').run(sqlitePath(dir));
      try {
        for (const table of HISTORY_TABLES) {
          const theirs = new Set(db.prepare(`PRAGMA previous.table_info(${table})`).all().map(c => c.name));
          ...
          rows += Number(db.prepare(`INSERT OR IGNORE INTO main.${table} (${columns})
            SELECT ${columns} FROM previous.${table} WHERE project_id = ?`).run(projectId).changes);
        }
      } finally { db.exec('DETACH DATABASE previous'); }
    } catch { continue; }
