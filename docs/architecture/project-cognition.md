...
flowchart LR
    ...
    H[Selective host observations and user corrections] --> T[Local telemetry]
...
An omitted fact means unknown, not false. Unknown assumptions remain visible as checks; a known mismatch suppresses the advice. A file or component outside the declared scope excludes the memory; an operation outside the declared ones does not, because authored operations (implement, review) and host-reported ones (edit, read, test) are different vocabularies, and a matching operation adds no activation of its own. A scope the request already verified is not restated in the advice. A changed evidence file keeps the advice visible in micro form, prefixed `EVIDENCE CHANGED since review`; a due revision condition or verified counterevidence yields a review notice without repeating the potentially stale instruction. Validity windows exclude future or expired guidance.
...
Knowledge also leaves the effective set without being replaced. A memory is retired to `archived` when it was never reviewed above `inferred`, was never activated, is past a minimum age, and the project ran enough retrievals after it was written that its trigger had real chances to fire and did not. Retirement is by disuse, never by age alone, and never by recency decay: an unused lesson about a quiet subsystem stays. Review protects — `validated` and `canonical` are out of reach — and anything currently before a reviewer is excluded: a contested memory, because a dispute is an open human question rather than disuse, and a memory a pending candidate names in `replaces`, because retiring it would leave a revision that review can never approve. The sweep runs lazily on the write path, which already holds the lock; it never runs on the hot path, and it cannot fail the write that triggered it. Archiving is not deletion: the file moves to the archive, the memory leaves retrieval and the effective-set uniqueness predicate, and a reviewer restores it from the audit UI unless another memory has since taken its topic. Deletion stays manual and confirmed.
...
File, diff and test-log references must resolve inside the consuming repository, including after symlink resolution. DD computes SHA-256 hashes and limits file reads. It does not execute referenced commands. A recorded observation, host or user correction, can be referenced by its ID with `source_type=tool_output` and must belong to the same project.
...
The resident adds an embedding activation (`src/semantic/provider.js`). It widens candidates and competes with lexical activation; every scope gate and floor still applies. A memory counts by how far it stands above the query's mean similarity, after discounting its hubness: its average closeness to the rest of the store. Without that discount a long memory that lists many nouns sits near every request and tops unrelated ones. Hubness comes from the store alone, never from past requests. `semantic.floor` in `.dd/config.json` trades silence for recall per project. Memories are authored in English; Spanish requests reach them mainly through authored trigger variants.
...
Full, short and micro forms are tried in order. A larger form is skipped when it would take more than half the payload budget, so one long rationale falls back to micro instead of crowding out the rest. `vpt_threshold` is a floor on a candidate's marginal value, not a ratio to its size: the ratio capped every memory near 50 tokens, so reviewed advice longer than a sentence was never injected. Disputes, unknown assumptions and review notices remain explicit. At most eight memories are returned. Memories a reviewer tags `ambient` (architecture, project-wide conventions) are sent once per session at SessionStart, in micro form within 150 tokens, and are not repeated by the tool-call path in that session.
...
Only failures, test, lint or type-check runs whose own exit status reports success and whose output reports no failures, and user prompts carrying corrective language become bounded local observations; normal reads, unrelated successful commands and ordinary requests are discarded. Correction detection is a deterministic English/Spanish regular expression evaluated inline on `UserPromptSubmit`, because the hook is an ephemeral process with two dependencies: there is no model to call and single-developer volumes give nothing to learn a classifier from. It records that a correction happened and what was said, never what it was about; connecting it to a behaviour is the model's job at proposal time. Detection promotes nothing, so a false positive costs one reviewable observation while a missed correction loses the most reliable signal DD receives. The observation is stamped with the `user_correction` provenance and carries no confidence of its own. Both the standalone hook process and the resident hook service record it, and a failure to record it never withholds the turn's retrieval context. The audit UI lists recorded observations read-only, so a reviewer can see what a proposal is citing; there is nothing to approve there, because promotion happens on the memory that cites the evidence, never on the evidence. The capture prompt can expose up to three observation IDs from that session, inspectable with `get`, and names host observations and user corrections as citable sources; this context bound does not limit the number of proposals. On every host, the Stop reminder requires recorded evidence from that turn — a host observation or a user correction — while ordinary turns remain quiet; on Codex it is also a continuation, so it costs an extra model turn. The reminder itself says most turns warrant no proposal. A corrected turn therefore spends a continuation, deliberately: it is the turn least worth staying quiet on, and the remembered evidence set still prevents the same correction prompting again. Observation retention and retrieval payload budgets remain separate from capture counts.
