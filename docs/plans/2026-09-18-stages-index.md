...
| 7 | [Forgetting](2026-09-18-stage-7-forgetting.md) | **Done** `9f89761` | `archived` is declared and unreachable; the effective set only grows | High |
...
Every stage is now **Done**. A stage that is next in a future series carries a *Corrections from stage
N* section: its *Start here* checks have already been run against the current code, and every place
the plan disagrees with what is there is written down. Start from that section, not from the plan
body.
